...
export function formatBrazilianPhone(value: string) {
  ...
  if (!normalized) return value.replace(/\D/g, '') || value
...
export function maskBrazilianPhone(value: string) {
  ...
  if (displayDigits.length <= 2) return displayDigits ? `(${displayDigits}` : ''
  if (displayDigits.length <= 6) return `(${displayDigits.slice(0, 2)}) ${displayDigits.slice(2)}`
  if (displayDigits.length <= 10) return `(${displayDigits.slice(0, 2)}) ${displayDigits.slice(2, 6)}-${displayDigits.slice(6)}`
