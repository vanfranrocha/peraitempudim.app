-- Criação pública segura de pedidos via RPC.
-- Não libera INSERT anon direto nas tabelas; a função é a única porta pública.
...
begin
  ...
  loop
    ...
    v_unit_price := case
      when v_product.is_on_promotion and v_product.promotional_price is not null then v_product.promotional_price
